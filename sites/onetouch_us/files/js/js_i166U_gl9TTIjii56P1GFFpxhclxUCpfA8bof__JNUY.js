...
;
Drupal.behaviors.onetouch_find_pharmacy={attach:function(a){function b(a,b){let c=b?"$1zip="+b:"";if(d.test(a))a=a.replace(d,c);else if(""!==c){let b="?";0<=a.indexOf("?")&&(b="&"),a+=c.replace("$1",b)}return a}let c=jQuery,d=/(.)\bzip\=([^\&]*)/;c(document).ready(()=>{function b(){let a=d.val(),b=c(".form-item--error-message");b.hide(),f.attr("data-attribute-datalayer-event-action","rx-finder: "+a),f.attr("data-attribute-datalayer-event-label","rx-finder: "+a)}let d=c("[name=zip]",a),e=c(d.closest("form"),a),f=e.find("[type=submit]"),g=e.find("[type=submit]").parent();d.on("change",b),d.on("keyup",b),d.on("focus",b),e.on("submit",b),f.on("click",b),g.on("click",()=>{5==d.val().length&&(b(),f.click())})}),c(a).ajaxStart(function(){let d=location.href,e=b(d,c("[name=zip]",a).val());d!==e&&history.pushState({},"",e)})}};;
/*!*
